...
const ImportRoomsButton = ({ hotelId, disabled, onSuccess }) => {
  ...
  const handleFetchPreview = async (e) => {
    ...
    try {
      ...
      if (!rawRows || rawRows.length === 0) {
        throw new Error("The sheet/CSV file contains no room data rows.");
      }

      // Dry Run (Preview API) - send JSON array to backend preview endpoint
      const res = await api.post("/import/rooms/preview", {
        hotelId,
        rawRows,
      });
      ...
      setErrorMsg(
        err.response?.data?.message || err.message || "Failed to process room preview dry run.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      const res = await api.post("/import/rooms/confirm", payload);

      alert(
        res.data.message ||
          `Rooms imported successfully! ${res.data.createdCount} created, ${res.data.updatedCount} updated.`,
      );
      ...
      setErrorMsg(
        err.response?.data?.message || "Failed to confirm rooms import.",
      );
    } finally {
      setConfirming(false);
    }
  ...
  return (
    <>
      ...
      >
        ...
        <span>Import Rooms (.csv / Sheet)</span>
      </button>
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1 font-light">
                  {step === 1
                    ...
                    : "Verify parsed room numbers, resolve duplicate rooms, and confirm."}
                </p>
              </div>
                      ...
                      >
                        {loading ? (
                          <>
                            ...
                            <span>Parsing & Checking Rooms...</span>
                          </>
                        ) : (
                          "Preview Rooms & Check Duplicates"
                        )}
                      </button>
                    </div>
                  </form>
                  ...
                  {previewData.summary.duplicateCount > 0 && (
                    <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex items-center gap-2 text-amber-700 dark:text-amber-400 text-xs font-semibold">
                        <ExclamationTriangleIcon className="w-5 h-5 shrink-0" />
                        <span>Duplicate room numbers exist in your hotel! Choose how to handle them:</span>
                      </div>
                      ...
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="bg-neutral-100 dark:bg-neutral-950 text-neutral-500 dark:text-neutral-400 font-bold uppercase tracking-wider border-b border-neutral-200 dark:border-neutral-800 sticky top-0 z-10">
                            ...
                            <th className="p-3.5">Room Name & Type</th>
                            <th className="p-3.5">Price / Night</th>
                          ...
                          {previewData.items.map((item) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* ROOM NO */}
                                <td className="p-3.5 align-top font-semibold text-neutral-900 dark:text-white whitespace-nowrap">
                                  {isEditing ? (
                                    <input
                                      type="text"
                                      value={editFormData.roomNumber || ""}
                                      onChange={(e) => setEditFormData({ ...editFormData, roomNumber: e.target.value })}
                                      className="px-2 py-1 rounded border border-blue-500 dark:bg-neutral-950 font-mono text-xs w-24"
                                    />
                                  ) : (
                                    <span>Room #{item.data.roomNumber}</span>
                                  )}
                                </td>
                                  ...
                                  {isEditing ? (
                                    ...
                                    <div className="space-y-1">
                                      <p className="font-medium text-neutral-900 dark:text-white">
                                        {item.data.roomName || "Standard Room"} ({item.data.roomType})
                                      </p>
                                      {item.existingData && (
                                        <div className="p-1.5 rounded-lg bg-amber-100/60 dark:bg-amber-900/30 text-[11px] text-amber-800 dark:text-amber-300 font-mono">
                                          <strong>Existing DB Room:</strong> #{item.existingData.roomNumber} - {item.existingData.roomType} (₹{item.existingData.pricePerNight})
                                        </div>
                                      )}
                                    </div>
                                  )}
                                ...
                                {/* PRICE */}
                                <td className="p-3.5 align-top whitespace-nowrap font-medium text-neutral-900 dark:text-white">
                                  {isEditing ? (
                                    <input
                                      type="number"
                                      value={editFormData.pricePerNight || 0}
                                      onChange={(e) => setEditFormData({ ...editFormData, pricePerNight: Number(e.target.value) })}
                                      className="px-2 py-1 border rounded dark:bg-neutral-950 w-24"
                                    />
                                  ) : (
                                    <span>₹{item.data.pricePerNight}</span>
                                  )}
                                </td>
...
export default ImportRoomsButton;
