...
const Navbar = () => {
  ...
  const renderAvatar = () => {
    ...
    return (
      <div className="w-10 h-10 rounded-full bg-black dark:bg-white text-white dark:text-black font-semibold flex items-center justify-center text-sm shadow-sm transition-transform active:scale-95">
        {initial}
      </div>
    );
  };
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-white/80 dark:bg-neutral-950/80 backdrop-blur-xl border-b border-neutral-200 dark:border-neutral-800 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        {/* Brand Logo */}
        <div className="font-serif text-2xl tracking-tight text-neutral-900 dark:text-white">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-full bg-neutral-100 dark:bg-neutral-900 text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-colors cursor-pointer"
          >
            {theme === "dark" ? (
          ...
          {isAuthenticated ? (
            <div className="relative" ref={dropdownRef}>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl shadow-black/10 dark:shadow-black/50 border border-neutral-100 dark:border-neutral-800 py-2 z-50 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="px-4 py-3.5 border-b border-neutral-100 dark:border-neutral-800">
                    ...
                    <div className="inline-block px-2 py-1 bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 text-[10px] font-bold uppercase tracking-widest rounded-md">
                      {formatRole(user?.role)}
                    </div>
                  </div>

                  <div className="py-2">
                    {/* Dashboard Highlighted for Admins/Hotels */}
                    {dashboardRoute && (
                      <div className="border-b border-neutral-100 dark:border-neutral-800 pb-1 mb-1">
                        <Link
                          to={dashboardRoute}
                          onClick={() => setDropdownOpen(false)}
                          className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                        >
            ...
            <Link
              to="/login"
              className="bg-black dark:bg-white text-white dark:text-black hover:opacity-80 px-6 py-3 rounded-full text-sm font-semibold transition-all shadow-md active:scale-95"
            >
              Login
            </Link>
          )}
        ...
        <div className="flex md:hidden items-center gap-3">
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-full bg-neutral-100 dark:bg-neutral-900 text-neutral-600 dark:text-neutral-400"
          >
            {theme === "dark" ? (
              <SunIcon className="w-5 h-5 text-yellow-400" />
            ) : (
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-neutral-100 dark:bg-neutral-900 text-neutral-700 dark:text-neutral-300"
          >
            {mobileMenuOpen ? (
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800 px-6 py-6 space-y-4 animate-in slide-in-from-top duration-300">
          ...
          {isAuthenticated ? (
            <>
              {/* Mobile Profile Header with Role */}
              <div className="pt-4 border-t border-neutral-100 dark:border-neutral-800 flex items-center gap-3 mb-4">
                {renderAvatar()}
                <div>
              ...
              {/* Dynamic Role Dashboard Link (Mobile) */}
              {dashboardRoute && (
                <Link
                  to={dashboardRoute}
                  onClick={() => setMobileMenuOpen(false)}
