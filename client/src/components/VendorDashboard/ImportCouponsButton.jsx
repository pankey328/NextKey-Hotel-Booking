...
const ImportCouponsButton = ({ hotelId, disabled, onSuccess }) => {
  ...
  const handleFetchPreview = async (e) => {
    ...
    try {
      ...
      if (!rawRows || rawRows.length === 0) {
        throw new Error("The sheet/CSV file contains no coupon data rows.");
      }

      // Dry Run (Preview API) - send JSON array to backend preview endpoint
      const res = await api.post("/import/coupons/preview", {
        hotelId,
        rawRows,
      });
      ...
      setErrorMsg(
        err.response?.data?.message || err.message || "Failed to process coupon preview dry run.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      const res = await api.post("/import/coupons/confirm", payload);

      alert(
        res.data.message ||
          `Coupons imported successfully! ${res.data.createdCount} created, ${res.data.updatedCount} updated.`,
      );
      ...
      setErrorMsg(
        err.response?.data?.message || "Failed to confirm coupons import.",
      );
    } finally {
      setConfirming(false);
    }
  ...
  return (
    <>
      ...
      >
        ...
        <span>Import Coupons (.csv / Sheet)</span>
      </button>
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1 font-light">
                  {step === 1
                    ...
                    : "Verify parsed coupon codes, resolve duplicate codes, and confirm."}
                </p>
              </div>
                    ...
                    {activeTab === "file" ? (
                      <div className="border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl p-8 text-center hover:border-blue-500 dark:hover:border-blue-400 transition-colors bg-neutral-50/50 dark:bg-neutral-950/40">
                        ...
                        <input
                          type="file"
                          accept=".csv"
                          id="coupons-csv-input"
                          className="hidden"
                          onChange={(e) => setSelectedFile(e.target.files[0] || null)}
                        />
                        <label
                          htmlFor="coupons-csv-input"
                      ...
                      >
                        {loading ? (
                          <>
                            ...
                            <span>Parsing & Checking Coupons...</span>
                          </>
                        ) : (
                          "Preview Coupons & Check Duplicates"
                        )}
                      </button>
                    </div>
                  </form>
                  ...
                  {previewData.summary.duplicateCount > 0 && (
                    <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex items-center gap-2 text-amber-700 dark:text-amber-400 text-xs font-semibold">
                        <ExclamationTriangleIcon className="w-5 h-5 shrink-0" />
                        <span>Duplicate coupon codes exist in database! Choose how to handle them:</span>
                      </div>
                      ...
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="bg-neutral-100 dark:bg-neutral-950 text-neutral-500 dark:text-neutral-400 font-bold uppercase tracking-wider border-b border-neutral-200 dark:border-neutral-800 sticky top-0 z-10">
                            ...
                            <th className="p-3.5">Discount Details</th>
                          ...
                          {previewData.items.map((item) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* COUPON CODE */}
                                <td className="p-3.5 align-top font-mono font-bold text-neutral-900 dark:text-white whitespace-nowrap">
                                  {isEditing ? (
                                    <input
                                      type="text"
                                      value={editFormData.code || ""}
                                      onChange={(e) => setEditFormData({ ...editFormData, code: e.target.value.toUpperCase() })}
                                      className="px-2 py-1 rounded border border-blue-500 dark:bg-neutral-950 font-mono text-xs w-32 uppercase"
                                    />
                                  ) : (
                                  ...
                                  {isEditing ? (
                                    <div className="grid grid-cols-3 gap-2">
                                      ...
                                      <input
                                        ...
                                        value={editFormData.maxDiscount || 0}
                                      ...
                                      <input
                                        ...
                                        value={editFormData.minPrice || 0}
                                        onChange={(e) => setEditFormData({ ...editFormData, minPrice: Number(e.target.value) })}
                                        className="px-2 py-1 border rounded dark:bg-neutral-950"
                                      />
                                    </div>
                                  ) : (
                                    <div className="space-y-1">
                                      <p className="font-medium text-neutral-900 dark:text-white">
                                        {item.data.discount}% OFF (Max ₹{item.data.maxDiscount}, Min spend ₹{item.data.minPrice})
                                      </p>
                                      {item.existingData && (
                                        <div className="p-1.5 rounded-lg bg-amber-100/60 dark:bg-amber-900/30 text-[11px] text-amber-800 dark:text-amber-300 font-mono">
                                          <strong>Existing DB Coupon:</strong> {item.existingData.code} - {item.existingData.discount}% OFF (Max ₹{item.existingData.maxDiscount})
                                        </div>
                                      )}
                                    </div>
                                  )}
                                ...
                                {/* VALIDITY DATES */}
                                <td className="p-3.5 align-top whitespace-nowrap text-neutral-600 dark:text-neutral-400">
                                  {item.data.availFrom} to {item.data.expiryDate}
                                </td>
                    ...
                    >
                      {confirming ? (
                        <>
                          ...
                          <span>Importing Coupons...</span>
                        </>
                      ) : (
                        "Confirm & Import Coupons"
                      )}
                    </button>
                  </div>
                </div>
...
export default ImportCouponsButton;
